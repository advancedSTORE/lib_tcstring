...
impl TryFrom<&str> for TCModelV1 {
    ...
    fn try_from(val: &str) -> Result<Self, Self::Error> {
        match base64::decode_config(val, base64::URL_SAFE_NO_PAD) {
            ...
            Err(_) => Err(INVALID_URL_SAFE_BASE64),
        }
    }
}

impl TCModelV1 {
    fn try_from_vec(val: Vec<u8>) -> Result<Self, TCSDecodeError> {
        ...
        Ok(Self {
            ...
            purpose_consents: parse_u8_bitfield_from_bytes(&val, 132, 24)?,
    ...
    fn iab_tcf_v1_allowed_vendor_range_1() {
        assert_eq!(
                ...
                Ok(TCModelV1 {
                    ...
                    purpose_consents: vec![1, 2, 3, 4, 5],
                    vendors: VendorSet {
                        is_blocklist: false,
